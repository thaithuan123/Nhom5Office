...
const Cart = () => {
  ...
  const [promoCode, setPromoCode] = useState(() => {
    if (typeof window !== 'undefined') {
      return window.localStorage.getItem('phonehub_selected_promo') || '';
    }
    return '';
  });
  ...
  useEffect(() => {
    ...
    const persistedPromo = typeof window !== 'undefined' ? window.localStorage.getItem('phonehub_selected_promo') : '';
    if (persistedPromo) {
      validatePromotionCode(persistedPromo);
    }
    loadPromotions();
  }, []);
  ...
  useEffect(() => {
    if (selectedPromo) {
      ...
      const calculatedDiscount = calculateDiscount(selectedPromo, totalBefore);
      setDiscountAmount(calculatedDiscount);
    }
  }, [selectedPromo, cartItems]);
  ...
  useEffect(() => {
    if (cartItems.length === 0) {
      ...
      setPromoMessage('');
      if (typeof window !== 'undefined') {
        window.localStorage.removeItem('phonehub_selected_promo');
      }
    }
  }, [cartItems]);
  ...
  const calculateDiscount = (promotion, totalAmount) => {
    ...
    return Math.min(Math.floor(totalAmount * (value / 100)), totalAmount);
  };
  ...
  const validatePromotionCode = async (code) => {
    ...
    setPromoMessage('Đang kiểm tra mã...');

    try {
      const response = await fetch(`${API_BASE}/api/promotions/validate?code=${encodeURIComponent(normalizedCode)}`);
      const data = await response.json();
      if (!response.ok) {
      ...
      const totalBefore = getTotalPrice();
      const discount = calculateDiscount(data, totalBefore);
      setSelectedPromo(data);
      setPromoCode(data.code);
      setDiscountAmount(discount);
      setPromoMessage(`Áp dụng ${data.code} thành công, tiết kiệm ${discount.toLocaleString('vi-VN')} ₫`);
      if (typeof window !== 'undefined') {
        window.localStorage.setItem('phonehub_selected_promo', data.code);
      }
    } catch (err) {
      setSelectedPromo(null);
      setDiscountAmount(0);
      setPromoMessage(err.message || 'Mã giảm giá không hợp lệ');
      if (typeof window !== 'undefined') {
        window.localStorage.removeItem('phonehub_selected_promo');
      }
    } finally {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/orders`, {
        ...
        body: JSON.stringify({
          ...
          promoCode: selectedPromo?.code || promoCode,
        }),
      });
          ...
          <div className="cart-summary">
            <div className="summary-box">
              ...
              <div className="summary-item total">
                <span>Tổng cộng:</span>
                <span>{Math.max(0, getTotalPrice() - discountAmount).toLocaleString('vi-VN')} ₫</span>
              </div>
              ...
              <div className="promo-box">
                ...
                <div className="promo-input-group">
                  ...
                  <button
                    ...
                    onClick={() => validatePromotionCode(promoCode)}
                ...
                {promoMessage && (
                  <p className={`promo-message ${selectedPromo ? 'success' : 'error'}`}>
                    {promoMessage}
                  </p>
                )}
                ...
                {selectedPromo && (
                  <div className="selected-promo">
                    <strong>{selectedPromo.code}</strong> - {selectedPromo.title}
                    <button type="button" className="btn btn-link" onClick={clearPromotion}>
                      Xóa mã
                    </button>
                  </div>
                )}
                  ...
                  <div className="promo-list">
                    ...
                    <div className="promo-list-grid">
                      {availablePromotions.slice(0, 3).map((promotion) => (
                        <div key={promotion.id} className="promo-card">
                          <div className="promo-card-header">
                            <span className="promo-code">{promotion.code}</span>
                            <span className="promo-type">{promotion.discountType === 'fixed' ? `${Number(promotion.discountValue).toLocaleString('vi-VN')} ₫` : `${promotion.discountValue}%`}</span>
                          </div>
                          <p className="promo-title">{promotion.title}</p>
                          <p className="promo-description">{promotion.description}</p>
                          <button type="button" className="btn btn-secondary" onClick={() => selectPromotion(promotion)}>
                            Chọn mã
                          </button>
                        </div>
                      ))}
